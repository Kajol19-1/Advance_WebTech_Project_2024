<a class="btn btn-primary" href="{{route('contact')}}"> Contact </a> 
<a class="btn btn-primary" href="{{route('profile')}}"> Profile </a>
<a class="btn btn-primary" href="{{route('student.create')}}"> Create Student </a>
<a class="btn btn-primary" href="{{route('student.list')}}"> Student List </a>
<a class="btn btn-primary" href="{{route('teacher.create')}}"> Create Teacher </a>
<a class="btn btn-primary" href="{{route('teacher.list')}}"> Teacher List </a>

<a class="btn btn-danger" href="{{route('home')}}"> Home </a>
